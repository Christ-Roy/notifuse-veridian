...
// Veridian — les débits par classe de provider destinataire par défaut du
// workspace doivent survivre au round-trip JSON (la colonne settings est du
// JSONB intégral) et rester absents quand non configurés (les workspaces
// existants ne portent pas la clé et ne doivent pas changer de forme).
func TestWorkspaceSettings_VeridianProviderClassRatesRoundTrip(t *testing.T) {
	t.Run("rates survive JSON round-trip", func(t *testing.T) {
		settings := WorkspaceSettings{
			Timezone:                   "Europe/Paris",
			VeridianProviderClassRates: map[string]float64{"google": 1, "microsoft": 2.5},
		}
		...
		var decoded WorkspaceSettings
		require.NoError(t, json.Unmarshal(raw, &decoded))
		assert.Equal(t, settings.VeridianProviderClassRates, decoded.VeridianProviderClassRates)
	})
